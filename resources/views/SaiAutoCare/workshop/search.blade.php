...
// for refresh : start
     $(document).on('click','.openRefreshBalance',function()
    {
          var id = $(this).attr('id')
               ...
               $.ajax({
              type:"GET",
              url: "{{  url('/') }}/SaiAutoCare/sale/edit/"+id,
              ...
              success: function(data){
                 ...
                 if(data==1)
                 {
                  ...
                  $('[name=amountForWorkshopDiscount]').val("");      
                   var newTab = window.open("{{url('/')}}/SaiAutoCare/workshop/view/"+workshopIdForDiscount, "_blank");
                      newTab.location;
              ...
              success: function(data){
                 ...
                 if(data==1)
                 {
                  swal("Good job!", "Workshop Payment  Successfully", "success");
                      $('[name=amountForWorkshop]').val("");
                       var newTab = window.open("{{url('/')}}/SaiAutoCare/workshop/view/"+workshopIdForPayment, "_blank");
                      newTab.location;
                   console.log("Worshop Detail Opened In New Tab");
                 }
                 else if (data==0){
